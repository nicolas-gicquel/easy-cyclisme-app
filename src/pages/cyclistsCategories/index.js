          ...
          <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div className="flex items-center justify-end mt-4">
              <a href="/cyclistsCategories/add">
                <Button className="ml-4 mr-4">Créer une nouvelle catégorie</Button>
              </a>
            </div>
            ...
            <div className="p-6 bg-white border-b border-gray-200">
              ...
              <table class="border-collapse table-auto w-full text-sm">
                <thead>
                  <tr>
                    <th class="border-b dark:border-slate-600 font-medium p-4 pr-8 pt-0 pb-3 text-slate-400 dark:text-slate-200 text-left">Nom de la catégorie</th>
                ...
                <tbody class="bg-white dark:bg-slate-800">
                  {cyclistsCategories.map((cyclistsCategory) => (
                    <tr>
                      <td class="border-b border-slate-100 dark:border-slate-700 p-4 text-slate-500 dark:text-slate-400">{cyclistsCategory.name_cyclists_category}</td>
                      <td class="border-b border-slate-100 dark:border-slate-700 p-4 text-slate-500 dark:text-slate-400"><p className="mt-1 text-sm text-gray-500 text-center">
                        <a href={`/cyclistsCategories/edit/${cyclistsCategory.id}`}>Editer </a>
                        <button onClick={
                          () => {
                            deleteCompetition(cyclistsCategory.id);
                          }
                        }
                        >
                          Supprimer</button>
